...
impl IsvEnclaveReport {
    ...
    pub fn misc_select(&self) -> Result<MiscSelect, QuoteError> {
        MiscSelect::from_bits(self.misc_select).ok_or(QuoteError::InvalidMiscSelect)
    }

    /// Attributes of the enclave (Section 38.7.1)
    pub fn attributes(&self) -> Result<Attributes, QuoteError> {
        let features = Features::from_bits(u64::from_le_bytes(self.features))
            .ok_or(QuoteError::InvalidFeatures)?;
        let xfrm = Xfrm::from_bits(u64::from_le_bytes(self.xfrm)).ok_or(QuoteError::InvalidXfrm)?;
